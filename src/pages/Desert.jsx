  ...
  return (
    <div className="container w-[390px] pl-3 pr-3 pb-10 bg-white pt-5 mx-auto flex flex-col gap-5  ">
      <div className="flex items-center justify-between ">
        <h1 className="text-[28px] font-semibold ">Десерты</h1>
      </div>
      <div className="flex flex-col items-center gap-5">
        {products
          .filter((value) => value.category === "Десерты")
          .map((desert) => (
            <div className="w-[350px] gap-2  h-[124px] flex items-center border-[#15141446] rounded-[5px] border">
              <div className="min-w-25 h-25">
                <img className="w-25 h-25" src={desert.image} alt="" />
              </div>
              <div className="flex gap-2 flex-col">
                <h1 className="text-[14px] font-bold">{desert.name}</h1>
                <p className="text-[12px]">{desert.description}</p>
